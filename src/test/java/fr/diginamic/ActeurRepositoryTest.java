...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles WHERE nom = 'Harley QUINN'", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles r JOIN r.film f WHERE annee = 2015", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.realisateurs r WHERE r.identite = 'Ridley Scott' AND f.annee BETWEEN 2010 AND 2020", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
